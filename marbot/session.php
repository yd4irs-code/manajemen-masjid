	...
	public function verification($id){
		...
		//Jika request proses ditemukan______________________________________________________________
			...
			if (isset($_SESSION['role']) && $_SESSION['role'] === 'bendahara') {
				$allowed = ['keuangan', 'formSave', 'formDelete', 'logout'];
				if (!in_array($id, $allowed)) {
					$this->retError("Akses ditolak: Bendahara hanya dapat mengakses menu keuangan.");
				}
			}
			if (isset($_SESSION['role']) && $_SESSION['role'] === 'marbot') {
				$allowed = ['info', 'running_text', 'wallpaper', 'petugas_jumat', 'keuangan', 'formSave', 'formDelete', 'logout', 'getWallpaper', 'saveWallpaper', 'wallpaperDelete'];
				if (!in_array($id, $allowed)) {
					$this->retError("Akses ditolak: Marbot tidak memiliki izin ke menu ini.");
				}
			}
			$this->id	= $id;
